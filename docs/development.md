...
- 卫星筛选：天空图提供 GPS/BDS/GLO/GAL/QZSS/GNSS 星座过滤开关，并支持 CN0、方位角、俯仰角区间筛选；关闭或区间外的卫星不参与天空图、CN0 TOP7 和 CN0 统计分析。
- NMEA 文件：支持导出接收到的原始 NMEA，支持导入离线 NMEA 并按回放速度播放；回放进度条支持拖动定位到指定语句并刷新显示。
- 回放 seek：拖动进度条时仅更新目标值，释放进度条后执行 seek；seek 时重置解析器并从文件起点重建到目标位置，避免拖动过程中的 UI 重入和 CN0/GSV 状态丢失。
...
- 工程报告：支持直接导入 NMEA 做批量分析并导出 HTML 摘要报告和 CSV 明细；如果已载入标准轨迹则输出精度结果。
- NMEA/KML 转换：支持从 NMEA 定位轨迹导出 KML，也支持从 KML 坐标生成基础 GGA/RMC NMEA 文件。
- 功能简介：界面顶部提供 `Help` 入口，以工程软件快速上手说明形式介绍串口实时测试、离线回放、天空图筛选、地图瓦片、报告导出、NMEA/KML 转换和自动化指令测试。
...
- 如果已导入标准轨迹，报告输出水平 RMS/MAX/CEP50/CEP95 与高程 RMS/MAX。
- 报告按秒聚合 GNSS 历元，输出每秒卫星跟踪总数、各星座跟踪数、估算星历个数、定位实际使用卫星数、HDOP/PDOP/VDOP。
- 报告按星座输出每秒 TOP5 CN0、估算星历数和使用卫星数。
- 报告输出速度和航向角随时间变化图表。
- 当前 NMEA 标准语句无法直接给出严格星历状态时，星历个数用 `CN0 > 0` 的已跟踪卫星数量作为工程估算，并在报告中标注。

NMEA/KML 转换：

- NMEA 转 KML 时只导出有效定位点，KML 使用 `lon,lat,alt` 坐标顺序。
- KML 转 NMEA 时读取 `coordinates` 中的轨迹点，生成基础 `GGA/RMC` 语句并自动计算校验和；时间戳从当前 UTC 秒开始递增。
- 转换动作只读写用户选择的本地文件，不联网、不上传轨迹。
...
- 顶部状态栏提供 `Help` 按钮，打开后展示各功能简介和典型使用流程，避免用户首次打开程序不知道如何开始。
...
- 天空图默认不显示 `CN0=0` 的卫星点；CN0、方位角和俯仰角区间筛选使用数值控件，筛选范围立即同步到天空图、CN0 TOP7 和分析摘要。
